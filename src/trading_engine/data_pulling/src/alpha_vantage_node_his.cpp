...
private:
    ...
    void fetch_stock_data(const std::string& symbol) {
        if(curl_) {
            ...
            // Modified URL for fetching daily historical data
            std::string url = "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=" + symbol + "&apikey=" + api_key_;
                ...
                try {
                    ...
                    if (j.contains("Time Series (Daily)")) {
                        ...
                        message.data = 
                            ...
                            "time: " + timestamp + "\n" +
                            "volume: " + data["5. volume"].get<std::string>() + "\n";
                        
                        publisher_->publish(message);
                        RCLCPP_INFO(this->get_logger(), "Published data for %s on %s", symbol.c_str(), timestamp.c_str());
