...
public:
    AlpacaTradingNode() : Node("alpaca_trading_node") {
        ...
        subscription_ = this->create_subscription<std_msgs::msg::String>(
            "trading_commands", 10, std::bind(&AlpacaTradingNode::command_callback, this, std::placeholders::_1));
    }
    ...
    void command_callback(const std_msgs::msg::String::SharedPtr msg) {
        ...
        try {
            ...
            if (command["action"] == "buy") {
                place_order(command["symbol"], command["quantity"], "buy", "market", "day");
            } else if (command["action"] == "sell") {
                place_order(command["symbol"], command["quantity"], "sell", "market", "day");
            } else {
