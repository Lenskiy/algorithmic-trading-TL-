...
class ServiceClient:
    ...
    def call(self, *args):
        ...
        request = self.pack_request(args)
    ...
    def call_async(self, *args):
        ...
        def func(_):
            ...
            result = self.unpack_response(response)
...
def create_order(last_tick_data: TickData, position_size: float, signal: str):
    if signal == "buy":
        side = "buy"
    elif signal == "sell":
        side = "sell"
    ...
    order = Order()
    order.symbol = last_tick_data.symbol
    order.exchange = last_tick_data.exchange
    order.price = last_tick_data.price
    order.size = position_size / last_tick_data.price
    order.time = last_tick_data.time
    order.side = side
